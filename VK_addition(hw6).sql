...
SELECT users.*
FROM users
LEFT JOIN messages ON messages.from_user_id = users.id OR messages.to_user_id = users.id
WHERE messages.id IS NULL;
CREATE TABLE users_old LIKE users;
-- Создание таблицы users_old
CREATE TABLE users_old LIKE users;
DELIMITER //
CREATE PROCEDURE move_user_to_users_old(IN user_id_to_move BIGINT UNSIGNED)
BEGIN
    ...
    IF user_data IS NULL THEN
        ...
        SET MESSAGE_TEXT = 'Пользователь с указанным id не найден в таблице users';
    ELSE
        INSERT INTO users_old (id, firstname, lastname, email)
        VALUES (
            JSON_UNQUOTE(JSON_EXTRACT(user_data, '$.id')),
            JSON_UNQUOTE(JSON_EXTRACT(user_data, '$.firstname')),
            JSON_UNQUOTE(JSON_EXTRACT(user_data, '$.lastname')),
            JSON_UNQUOTE(JSON_EXTRACT(user_data, '$.email'))
        );
        DELETE FROM users
        WHERE id = user_id_to_move;
        COMMIT;
